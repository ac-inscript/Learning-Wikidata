...
## Let's query!
Let us now explore some examples on how to query certain types of data. First we will consider a very basic query, then, as soon as we need other details to be investigated, we will add some more pieces of information to our query such as time periods and coordinates.  
Before stepping into the querying process, I shall make some remarks. Presently, Wikidata is not meant to be used as a search engine in the most general sense. I have tried myself to investigate banal things such as how many Archbishops of Canterbury have been or how many women were accused of witchcraft in certain countries. It is hard to obtain a result that approximates reality. The reason why is that we all think in many different ways and some items, properties or values may have been identified in different ways: some individuals ‘hold the position’ of archbishop, others have this role connected in Wikidata to property “occupation”, and still others are just Archbishops with a qualifier ‘Canterbury’. My recomendation would be then to use a good search engine for questions we don't know how to formulate (queries that can't be build without adding an infinite number of specifications). First, it seems to be better to know a project we want to investigate (there are plenty of them!) before querying Wikidata (some information might not have been stored yet), then see how the data set was built and which properties/items/values were used to describe the data. A research question about the witchcraft of whatever country could be well answered by the [WikiProject Scotland's Accused Witches](https://www.wikidata.org/wiki/Wikidata:WikiProject_Scotland%27s_Accused_Witches/Queries) (and notice, some of these broad projects come with a query suggestions page too).  
In other words, Wikidata is meant for researchers, and it is assumed that a researcher knows the 'domain' of the question.

1. https://w.wiki/FiWQ
2. https://w.wiki/FiWb
3. https://w.wiki/FiWi / https://w.wiki/FiWk
4. https://w.wiki/FiXD
5. https://w.wiki/FiYE
